...
NombresTablaPrecios = ["Material","Precio Compra","Precio Venta"]
...
df_filtrado_2 = crear_df_filtrado(df, predios_2, fecha_inicio, fecha_finalizacion, [], [])

pago = calcular_pago(df_filtrado_2, '1967', df_precio)
...
def filtrar_rutas(n_clicks,close_n_clicks,tab,refresh_n_clicks,predios, fecha_inicio, fecha_fin,legacy_id,data,sininfo_is_open,radio_button_val):
	"""
	...
	if trigger["prop_id"]=="search-button.n_clicks":
		if n_clicks>0:
			...
			df_filtrado = crear_df_filtrado(df, [predios], datetime.fromisoformat(fecha_inicio), datetime.fromisoformat(fecha_fin),[],[])
